...
def train_loop(config: _config.TrainConfig):
    ...
    global_step = 0
    if resuming:
        completed_step = load_ftp1_hpt_checkpoint(
            model, optim, config.checkpoint_dir, device, global_step=latest_step
        )
        # Checkpoints store the last finished step; the next optimizer update is +1.
        global_step = int(completed_step) + 1
        logging.info(
            f"Resumed training from completed step {completed_step}; next step is {global_step}"
        )
