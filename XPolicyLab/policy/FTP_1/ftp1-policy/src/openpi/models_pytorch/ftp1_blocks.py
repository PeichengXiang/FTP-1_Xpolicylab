...
class FTP1HptTactileEncoder(nn.Module):
    ...
    def _try_load_history_tokenizer_checkpoint(
        ...
        same_sensor_paths = sorted(
            path
            for path in checkpoint_dir.glob(f"{sensor_prefix}*.safetensors")
            if path.name != f"{tokenizer_key}.safetensors"
        )
        cross_sensor_paths = sorted(
            path
            for path in checkpoint_dir.glob("*.safetensors")
            if path.name not in {f"{tokenizer_key}.safetensors", "shared_image_chunk_encoder.safetensors"}
            and path not in same_sensor_paths
        )
        for candidate_path in [*same_sensor_paths, *cross_sensor_paths]:
