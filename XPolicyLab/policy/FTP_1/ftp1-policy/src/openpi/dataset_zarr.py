...
from openpi.ftp1_action_array_keys import resolve_ftp1_action_array_key
from openpi.ftp1_image_keys import parse_used_image_keys
from openpi.ftp1_image_keys import select_episode_image_keys
...
class ZarrDataset(Dataset):
    def __init__(
        ...
        self.used_image_keys = parse_used_image_keys(getattr(data_config, "used_image_keys", ""))
    ...
    def _get_single_arm_data(
        ...
        *,
        ...
        # ================================ Get Wrist Action ================================
        wrist_state_key = f"{hand_side}_wrist_pose"
        if wrist_state_key in data_key_set or f"{wrist_state_key}_action" in data_key_set:
            action_wrist_key = resolve_ftp1_action_array_key(data_key_set, wrist_state_key)
        ...
        arm_state_key = f"{hand_side}_arm_joints"
        if arm_state_key in data_key_set or f"{arm_state_key}_action" in data_key_set:
            action_arm_key = resolve_ftp1_action_array_key(data_key_set, arm_state_key)
        ...
        action_hand_key = resolve_ftp1_action_array_key(
            data_key_set, f"{hand_side}_hand_joints"
        )
    ...
    def __getitem__(self, idx: SupportsIndex, show_time: bool = False) -> Dict:
        ...
        # Find the first selected image key to determine dimensions.
        # used_image_keys=None keeps every present default camera; an explicit
        # allowlist (for example camera_ego_rgb) is single-view training.
        possible_image_keys = select_episode_image_keys(data_key_set, self.used_image_keys)
        base_image_key = None
        base_image = None
        ...
        for image_key in possible_image_keys:
            if image_key in data_key_set:
                ...
                # During normalization RGB is explicitly ignored. Read only
                # the Zarr metadata shape; indexing would decode every frame.
                base_image = None if self.ignore_rgb else data[image_key][idx]
                image_shape = data[image_key].shape[1:] if self.ignore_rgb else base_image.shape
                # Extract dimensions from image metadata or the sampled image.
                ...
                if len(image_shape) == 3:
                    if image_shape[0] == 3:
                        ...
                        raise ValueError(
                            f"Cannot determine image format from shape: {image_shape}, expected [H, W, C] or [C, H, W]"
                        )
                else:
                    raise ValueError(
