    ...
    def __getitem__(self, index):
        ...
        with h5py.File(dataset_path, 'r') as root:
            ...
            for cam_name in self.camera_names:
                ...
                if cam_name.endswith('tactile'):
                    size = (root.attrs['gelsight_height'], root.attrs['gelsight_width'])
                    gelsight_data = root[f'/observations/images/{cam_name}'][start_ts]/255.0
            ...
            all_cam_images = torch.stack(all_cam_images, dim=0) # (num_cams, c, h, w)
...
def get_norm_stats(dataset_dir, num_episodes, use_existing=True, chunk_size = 0):
    if os.path.exists(os.path.join(dataset_dir, 'norm_stats.json')) and use_existing:
        with open(os.path.join(dataset_dir, 'norm_stats.json'), 'r') as f:
            stats = json.load(f)
            print("Load norm stats from: ", os.path.join(dataset_dir, 'norm_stats.json'))
            print("!"*10)
            for k in stats:
        ...
        return {
            ...
            "gelsight_mean": (stats['left_tac_mean'] + stats['right_tac_mean']) / 2,
            "gelsight_std": (stats['left_tac_std'] + stats['right_tac_std']) / 2,
        }
    ...
    if use_gelsight:
        if use_existing and os.path.exists(os.path.join(dataset_dir, 'gelsight_norm_stats.json')):
            with open(os.path.join(dataset_dir, 'gelsight_norm_stats.json'), 'r') as f:
                ...
                gelsight_mean = np.array(gelsight_stats['left_tac_mean'])
                gelsight_std = np.array(gelsight_stats['left_tac_std'])
        else:
