...
- Treat multi-view image keys and their insertion order as part of the checkpoint contract. Training should use an explicit, fail-closed allowlist; `deploy.yml` must map the same model keys in the same order to the correct runtime camera sources and mark every configured view as required. When the checkpoint stores its training config, compare its image keys/history against the runtime map during adapter startup so a single-view checkpoint, swapped wrists, or a silently missing optional camera cannot reach inference.
